...
class Server:
    ...
    def proxy_thread(self, conn, client_addr):
        ...
        *******************************************
        ...
        requeststr = request.decode('utf8')

        # filter url
        if 'http://192.168.200.132:8001' in requeststr or 'http://192.168.200.131:8000' in requeststr:
            pass
        else:
            conn.close()
            return
        ...
        try:
            ...
            newrequest = request.decode('utf8').replace('http://192.168.200.132:8001', '')
            newrequest = newrequest.replace('http://192.168.200.131:8000', '').encode()
            s.sendall(newrequest)  # send request to webserver
